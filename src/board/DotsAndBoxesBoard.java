...
public class DotsAndBoxesBoard extends Board implements ExampleBoard{
    ...
    {
        totalBoxes = b;   //Total number of boxes gained by both players
    }
    ...
    public void printBoardState()
    {
        colour c = new colour();
        int i,j;
        char cl = 'A'; //to label the rows and columns of the board alphabetically
        char r = 'A';
        System.out.print("    ");
        for(j=0; j<getCols();j++)
        {
    ...
    public boolean makeMove(DotsAndBoxesPlayer player)
    {
        ...
        {
            error.invalidTypeInput("position/edge (eg. AA U)"); //prints an error message
            return true;
        }
        else {
        switch(player.edge) //A switch case according to which direction (U, D, L, R) the user inputted
        {
            case "U": 
