...
public class DotsAndBoxesBoard extends Board implements ExampleBoard{
    ...
    public void initializeBoard()
    {
        ...
        for(int i =0;i<getRows();i++){
            for(int j=0;j<getCols();j++){
                ...
                dotsBoard[i][j].s_piece = new Piece<>("0");
            }
        }
    }
        ...
        for (i=0; i<2*getRows()+1;i++)
        {
            ...
            for (j=0; j<2*getCols()+1;j++)
            {
                ...
                else if (i%2 == 1 && j%2 == 1)
                {
                    row = (i-1)/2;
                    col = (j-1)/2;
                    if (row < getRows() && col < getCols() && !dotsBoard[row][col].s_piece.getValueOnTile().equals("0")) 
                    {
                        System.out.print("  "+ dotsBoard[row][col].s_piece.getValueOnTile()+ "  ");
                    } 
                    else 
                    {
                        System.out.print("     ");
    ...
    { 
        ...
        for(j=getCols()-1;j>=0;j--)
        {
            ...
            {
                count++;
                dotsBoard[player.position[0]][j].s_piece.setValueOnTile(player.colour+player.getName().substring(0,1)+c.endColour);//Sets the player's initial on the tile if they claimed a box
